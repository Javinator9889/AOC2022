...
def advent_p1(input: str) -> str:
    ...
    Each monkey has several attributes:
        ...
        * Operation shows how your worry level changes as that monkey inspects an item. (An
          operation like new = old * 5 means that your worry level after the monkey inspected
    ...
    """
    monkeys: list[Monkey] = []
    input_line = iter(input.splitlines())
    while (line := next(input_line, None)) is not None:
        monkey: Monkey = {
            "inspected": 0,
        }  # type: ignore

        while line := next(input_line, None):
            key, value = line.split(":")
            key, value = key.strip(), value.strip()

            # we don't really care about first line
            if key.startswith("Monkey"):
                continue

            match key:
                case "Starting items":
                    items = deque([int(v) for v in value.split(",")])
                    monkey["items"] = items
                case "Operation":
                    real_op = value.split("=")[1].strip()
                    monkey["operation"] = compile(real_op, "<string>", "eval")
                case "Test":
                    divisor = int(value.split("divisible by")[1])
                    monkey["divisor"] = divisor
                case "If true" | "If false":
                    receptor_monkey = int(value.split("throw to monkey")[1])
                    monkey[key.split()[1]] = receptor_monkey  # type: ignore
    ...
    for _ in range(20):
        for monkey in monkeys:
            while len(monkey["items"]) > 0:
                ...
                worry_level = eval(monkey["operation"], {"__builtins__": {}}, {"old": item})
                worry_level = floor(worry_level / 3)
                dest_monkey = (
                    monkey["true"] if worry_level % monkey["divisor"] == 0 else monkey["false"]
                )
                monkeys[dest_monkey]["items"].append(worry_level)

    monkeys = sorted(monkeys, key=lambda monkey: monkey["inspected"], reverse=True)
    return str(monkeys[0]["inspected"] * monkeys[1]["inspected"])
...
def advent_p2(input: str) -> str:
    ...
    common_divisor = lcm.reduce([monkey["divisor"] for monkey in monkeys])
